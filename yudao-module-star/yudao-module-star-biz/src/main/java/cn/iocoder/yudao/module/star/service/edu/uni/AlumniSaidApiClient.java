...
public class AlumniSaidApiClient {
    ...
    public static void main(String[] args) {
        ...
        try (Session session = sessionFactory.openSession()) {
            // 查询所有 SchoolUniEntity 实体，且 isComment 为 null
            String hql = "FROM SchoolUniEntity WHERE isComment IS NULL";
            List<SchoolUniEntity> schoolList = session.createQuery(hql, SchoolUniEntity.class).getResultList();

            // 遍历每个学校名称
            for (SchoolUniEntity school : schoolList) {
                String keyword = school.getSchoolName(); // 获取学校名称作为 keyword
                int page = 1; // 页码
                            ...
                            for (JsonNode subitem : subitemArray) {
                                ...
                                if (subdisplayNode.isArray()) {
                                    for (JsonNode subdisplay : subdisplayNode) {
                                        ...
                                        // 保存到数据库
                                        try (Session sessionx = sessionFactory.openSession()) {
                                            sessionx.beginTransaction();
                                            sessionx.persist(comment);
