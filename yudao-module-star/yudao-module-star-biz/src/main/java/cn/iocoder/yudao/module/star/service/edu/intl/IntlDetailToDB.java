...
public class IntlDetailToDB {
    ...
    public static void main(String[] args) {
        try (Session session = sessionFactory.openSession()) {
            // 获取所有学校数据
            List<SchoolIntlEntity> schoolEntities = session.createQuery("FROM SchoolIntlEntity WHERE schoolProperties IS NULL ", SchoolIntlEntity.class).getResultList();

            ObjectMapper objectMapper = new ObjectMapper();
            objectMapper.registerModule(new JavaTimeModule());

            for (SchoolIntlEntity entity : schoolEntities) {
                ...
                try {
                    ...
                    if (responseMap.containsKey("code") && responseMap.get("code").equals(0) && responseMap.containsKey("data")) {
                        ...
                        SchoolIntlEntity updatedEntity = objectMapper.convertValue(dataMap, SchoolIntlEntity.class);
                        updatedEntity.setId(schoolId);

                        session.beginTransaction();
                        session.merge(updatedEntity);
                        session.getTransaction().commit();
                        System.out.println("学校 ID " + schoolId + " 数据更新成功");
                    } else {
                        System.err.println("学校 ID " + schoolId + " 接口返回 code 不为 0 或缺少 data 字段");
                        System.exit(1);
                    }
                } catch (Exception e) {
                    System.err.println("处理学校 ID " + schoolId + " 时出错: " + e.getMessage());
                    e.printStackTrace();
                    System.exit(1);
                }
                // 每条数据处理间隔 1 秒
